...
f_img <- mask.arr.box(tens = f_img, box.vec = v)
...
plot2D.arr(img.tens)
dmp <- plot2D.win.sig(tens = img.tens, center = c(3,4), Xspan = 1, Yspan = 1, k= 10)
f.dmp <- fft_2D(dmp, pl = 'amp')
f.dmp <- f.dmp[[1]]
f.dmp <- plot2D.zoom(f.dmp, center = c(0,0), Del_X = 100, Del_Y = 100)
f.dmp <- mask.arr.box(tens = f.dmp, box.vec = v)
dmp <- plot2D.boxes(img.tens = f.dmp, box.mat = b.mat, box.thick = 0.01, box_intens = 0.1)
#dmp <- plot2D.boxes(img.tens = f.dmp, box.mat = z.mat, box_intens = 0.1, box.thick = 0.01)
